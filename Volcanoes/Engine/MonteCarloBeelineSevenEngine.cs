...
{
    internal class MonteCarloBeelineSevenEngine : MonteCarloTreeSearchEngine
    {
            ...
            for (var i = 0; i < 80; i++)
            {
                if ((state.Tiles[i] > 0 && state.Player == Player.One) || (state.Tiles[i] < 0 && state.Player == Player.Two))
                {
                    ...
                    foreach (var tile in path)
                    {
                        ...
                        foreach (var neighbor in Constants.AdjacentIndexes[tile])
                        {
                            if (valid.Contains(neighbor) && !moves.Contains(neighbor))
                            {
                                moves.Add(neighbor);
                            }
                        }
